#!/usr/bin/env bash
# ============================================
# FluxPath MMU - FluxPath Installer v0.9.1-fixed
# Offline, production-ready, menu-driven installer
# RESPOND lines cleaned (no Jinja in MSG)
# ============================================
...
FLUX_INSTALLER_VERSION="0.9.1-fixed"
...
gcode:
  RESPOND PREFIX=MMU MSG="MMU_VARS loaded"
...
gcode:
  ...
  {% if lane < 1 or lane > v.mmu_lanes %}
    RESPOND PREFIX=MMU MSG="ERROR: Invalid lane"
  {% else %}
    SET_GCODE_VARIABLE MACRO=MMU_VARS VARIABLE=active_lane VALUE={lane}
    RESPOND PREFIX=MMU MSG="Active MMU lane set"
  {% endif %}
...
gcode:
  ...
  {% endif %}
  RESPOND PREFIX=MMU MSG="MMU_ENABLE executed"
...
gcode:
  ...
  RESPOND PREFIX=MMU MSG="MMU_PARK executed"
...
gcode:
  RESPOND PREFIX=MMU MSG="PRELOAD from pregate"
  MMU_MOVE_TO_CUTTER_FROM_PARK
  MMU_MOVE_TO_FILAMENT_SENSOR_EXPECT
  MMU_PARK
  RESPOND PREFIX=MMU MSG="PRELOAD complete"
EOF
...
gcode:
  RESPOND PREFIX=MMU MSG="LOAD start"
  {% set v = printer["gcode_macro MMU_VARS"] %}
  MMU_MOVE_TO_CUTTER_FROM_PARK
  MMU_MOVE_TO_FILAMENT_SENSOR_EXPECT
  ...
  G90
  RESPOND PREFIX=MMU MSG="LOAD complete"
EOF
...
gcode:
  RESPOND PREFIX=MMU MSG="UNLOAD start"
  {% set v = printer["gcode_macro MMU_VARS"] %}
  MMU_TIP_FORM
  ...
  MMU_CUT_SEQUENCE
  MMU_PARK
  RESPOND PREFIX=MMU MSG="UNLOAD complete"
EOF
...
gcode:
  ...
  {% set v = printer["gcode_macro MMU_VARS"] %}
  {% if lane < 1 or lane > v.mmu_lanes %}
    MMU_ERROR MSG="Requested lane out of range"
  {% endif %}
  RESPOND PREFIX=MMU MSG="TOOL_CHANGE start"
  MMU_UNLOAD
  MMU_SET_LANE LANE={lane}
  MMU_LOAD
  RESPOND PREFIX=MMU MSG="TOOL_CHANGE complete"
...
gcode:
  ...
  RESPOND PREFIX=MMU MSG="MMU ERROR"
  CANCEL_PRINT

[gcode_macro MMU_PREGATE_HIT]
gcode:
  RESPOND PREFIX=MMU MSG="PREGATE hit"
...
gcode:
  {% set v = printer["gcode_macro MMU_VARS"] %}
  RESPOND PREFIX=MMU MSG="MMU_VARS snapshot"
EOF
...
gcode:
  {% set lane = params.LANE|int %}
  MMU_SET_LANE LANE={lane}
  RESPOND PREFIX=MMU MSG="CAL: PARK→CUTTER lane"

[gcode_macro MMU_CAL_CUTTER_TO_FILAMENT_SENSOR]
gcode:
  RESPOND PREFIX=MMU MSG="CAL: Cutter→Sensor"

[gcode_macro MMU_CAL_FILAMENT_SENSOR_TO_EXTRUDER]
gcode:
  RESPOND PREFIX=MMU MSG="CAL: Sensor→Extruder"
EOF
...
gcode:
  ...
  RESPOND PREFIX=MMU MSG="DIAG: sensor test"

[gcode_macro MMU_DIAG_EXPECT_SENSOR]
gcode:
  RESPOND PREFIX=MMU MSG="DIAG: moving to sensor"
  MMU_MOVE_TO_CUTTER_FROM_PARK
  MMU_MOVE_TO_FILAMENT_SENSOR_EXPECT
  RESPOND PREFIX=MMU MSG="DIAG: sensor sequence done"
...
gcode:
  {% set lane = params.LANE|int %}
  MMU_SET_LANE LANE={lane}
  RESPOND PREFIX=MMU MSG="DIAG: lane test start"
  MMU_LOAD
  MMU_UNLOAD
  RESPOND PREFIX=MMU MSG="DIAG: lane test complete"
EOF
